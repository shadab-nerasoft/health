...
function NavItem({ icon: Icon, label, active = false }: { icon: typeof Home2; label: string; active?: boolean }) {
  return <button className={`nav-item ${active ? 'active' : ''}`}><Icon size="19" variant={active ? 'Bold' : 'Linear'} /><span>{label}</span></button>
}
...
export default function Page() {
  ...
  return <main className="wellness-app">
    <aside className="sidebar">
      <div className="brand"><div className="brand-mark"><Activity size="18" variant="Bold" /></div><span>wellnest</span></div>
      ...
      <div className="sidebar-bottom"><NavItem icon={Setting2} label="Settings" /><div className="user-chip"><div className="avatar">S</div><div><strong>Shadab</strong><span>Personal plan</span></div><ArrowRight2 size="16" /></div></div>
    </aside>
    <section className="content">
      <header className="topbar"><button className="mobile-menu" aria-label="Open menu"><HambergerMenu size="21" /></button><div className="mobile-brand">wellnest</div><div className="topbar-actions"><button className="icon-button" aria-label="Notifications"><Notification size="19" /></button><div className="avatar large">S</div></div></header>
      <div className="page-heading"><div><p className="eyebrow">Tuesday, October 24, 2024</p><h1>Good morning, Shadab</h1><p className="subheading">Here&apos;s your health at a glance.</p></div><button className="date-button">Today <ArrowRight2 size="16" /></button></div>
      <div className="dashboard-grid">
        <section className="hero-card"><div><p className="card-kicker">Today&apos;s movement</p><h2>Keep your rhythm</h2><p className="muted">You&apos;re building a great habit. A little more movement will get you to your goal.</p><button className="soft-button">View activity <ArrowRight2 size="16" /></button></div><StepRing /></section>
        <section className="metrics-grid"><article className="metric-card blue"><div className="metric-top"><span>Steps</span><Activity size="18" /></div><strong>8,420</strong><small>84% of daily goal</small><MiniBars values={[30, 45, 42, 65, 50, 72, 64, 88]} /></article><article className="metric-card peach"><div className="metric-top"><span>Calories</span><Flash size="18" /></div><strong>426 <em>kcal</em></strong><small>Estimated burned</small><MiniBars values={[42, 35, 52, 44, 68, 51, 60, 76]} color="var(--accent-peach)" /></article><article className="metric-card green"><div className="metric-top"><span>Distance</span><TrendUp size="18" /></div><strong>6.2 <em>km</em></strong><small>Distance walked</small><MiniBars values={[40, 55, 34, 62, 52, 76, 68, 80]} color="var(--accent-green)" /></article><article className="metric-card lavender"><div className="metric-top"><span>Active time</span><Chart2 size="18" /></div><strong>74 <em>min</em></strong><small>Active today</small><MiniBars values={[32, 45, 39, 58, 63, 52, 72, 66]} color="var(--accent-lavender)" /></article></section>
        ...
        <section className="insight-card"><div className="insight-icon"><Lamp size="20" /></div><div><p className="card-kicker">A little insight</p><h2>Your activity is highest between 6–8 PM.</h2><p className="muted">That evening window is becoming your strongest movement habit.</p></div></section>
        {!dismissed && <section className="recommendation"><div className="recommendation-icon"><Drop size="20" /></div><div><p className="card-kicker">Daily recommendation</p><h2>Stay hydrated</h2><p className="muted">You&apos;ve logged 1.2L today. Target: 2.5L</p></div><button onClick={() => setDismissed(true)} className="dismiss">Dismiss</button><ArrowRight2 size="18" /></section>}
      </div>
